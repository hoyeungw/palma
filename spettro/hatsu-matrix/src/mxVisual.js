...
export const mxVisual = (mx, { val: { min: m, dif: d }, pal: { min, dif }, naChalk, retFn, mutate }) => {
  ...
  return retFn
    ? map(mx, el => isNum(el) ? toHatsu(el, m, rto, min) : naChalk)
    : map(mx, el => isNum(el) ? el |> toHatsu(el, m, rto, min) : el |> naChalk)
}
...
export const mxVisualCol = (mx, y, { val: { min: m, dif: d }, pal: { min, dif }, naChalk, retFn, mutate }) => {
  ...
  const rto = mapAr(dif, _ => _ / d, 3)
  return retFn
    ? mapCol(mx, y, el => isNum(el) ? toHatsu(el, m, rto, min) : naChalk)
    : mapCol(mx, y, el => isNum(el) ? el |> toHatsu(el, m, rto, min) : el |> naChalk)
}
