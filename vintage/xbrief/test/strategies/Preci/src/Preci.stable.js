...
export class PreciStable {
  ...
  static fromArr (arr, head, tail, length) {
    if (!length) {
      if (!arr || !arr.length) return new PreciStable(null, null, false)
      length = arr.length
    }
    if (!head || head >= length) return new PreciStable(arr.slice(), null, false)
    if (!tail || tail >= length) return new PreciStable(arr.slice(0, head), null, true)
    if (head + tail >= length) return new PreciStable(arr.slice(0, head), arr.slice(-1), true)
    return new PreciStable(arr.slice(0, head), arr.slice(-tail), true)
  }
  ...
  map (fn, mutate = true) {
    ...
    return mutate
      ...
      : new PreciStable(head, tail, this.dash)
  }
